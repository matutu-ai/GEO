...
class StandardPathTests(unittest.TestCase):
    ...
    def test_14_standard_outputs_keyword_workbook(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with zipfile.ZipFile(output / "keyword_matrix.xlsx") as archive:
    ...
    def test_15_standard_outputs_persona_markdown(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = self._run_standard_cli(output)
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            content = (output / "persona_report.md").read_text(encoding="utf-8")
    ...
    def test_16_standard_outputs_traceable_keyword_fields(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = self._run_standard_cli(output)
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            matrix = json.loads((output / "keyword_matrix.json").read_text(encoding="utf-8"))
            required = {
    ...
    def test_17_legacy_interactive_still_passes(self):
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_18_legacy_fast_path_still_passes(self):
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_19_phase_0_contracts_still_validate(self):
    ...
    def test_20_standard_uses_fixed_stage_order(self):
